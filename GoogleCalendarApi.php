...
class GoogleCalendarApi
{
    ...
    public function GetCalendarsList($access_token)
    {
        ...
        $url_calendars =
            self::CALENDAR_LIST . '?' . http_build_query($url_parameters);
    ...
    public function CreateCalendarEvent(
        ...
        $event_date = !empty($event_datetime['event_date'])
            ? $event_datetime['event_date']
            : date('Y-m-d');
        $start_time = !empty($event_datetime['start_time'])
            ? $event_datetime['start_time']
            : date('H:i:s');
        $end_time = !empty($event_datetime['end_time'])
        ...
        if ($all_day == 1) {
            ...
            $timezone_offset = !empty($timezone_offset)
                ? $timezone_offset
                : '07:00';
            $dateTime_start =
                $event_date . 'T' . $start_time . $timezone_offset;
