    ...
    if (!empty($event_id)) {
        ...
        if (!empty($eventData)) {
            ...
            if (!empty($access_token_sess)) {
                ...
                $data = $GoogleCalendarApi->GetAccessToken(
                    GOOGLE_CLIENT_ID,
                    REDIRECT_URI,
            ...
            if (!empty($access_token)) {
                try {
                    ...
                    $user_timezone = $GoogleCalendarApi->GetUserCalendarTimezone(
                        $access_token
                    );
                    ...
                    if ($google_event_id) {
                        ...
                        $sqlQ =
                            'UPDATE events SET google_calendar_event_id=? WHERE id=?';
                        $stmt = $db->prepare($sqlQ);
                        $stmt->bind_param(
                            'si',
                            $db_google_event_id,
                        ...
                        $statusMsg =
                            '<p>Event #' .
                            $event_id .
                            ' has been added to Google Calendar successfully!</p>';
                        $statusMsg .=
                            '<p><a href="https://calendar.google.com/calendar/" target="_blank">Open Calendar</a>';
                    }
                } catch (Exception $e) {
